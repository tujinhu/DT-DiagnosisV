...
if __name__ == "__main__":
    ...
    fmode = '71-maneuverability-f3y-normal-r2vo'

    ulg_dir = sys.path[0] + '/' + fmode + '/ulg'  # 替换为存放ulg数据的文件夹路径
    data_dir = sys.path[0] + '/' + fmode + '/data'
    cmd_dir = sys.path[0] + '/' + fmode + '/cmd.txt'
    os.makedirs(data_dir, exist_ok=True)  # 创建data文件夹，如果不存在的话
    ...
    for index, ulg_file in enumerate(ulg_files, start=1):
        ...
        Dtrain.get_normal_train_data(log_folder, ntrain_folder, ctrlSeq)
